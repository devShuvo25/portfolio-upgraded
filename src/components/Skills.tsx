...
const Skills: React.FC = () => {
  ...
  return (
    <section 
      id="skills" 
      ref={containerRef}
      className="section-padding py-24 md:py-32 border-t border-[var(--border-color)] bg-[var(--bg-primary)]"
    >
      <div className="mb-16 md:mb-24">
        ...
        >
          <h2 className="text-xl md:text-2xl lg:text-5xl font-black mb-6 md:mb-8 uppercase font-display tracking-tighter text-[var(--text-primary)]">
            SKILLS<span className="text-[var(--accent-color)]">.</span>
          </h2>
          <p className="text-base md:text-lg font-bold uppercase tracking-[0.4em] text-[var(--text-tertiary)]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-8">
        {skills.map((skill, idx) => (
          <motion.div
            ...
            className="p-8 md:p-12 flex flex-col items-center justify-center gap-6 md:gap-8 group relative overflow-hidden rounded-[2rem] md:rounded-[3rem] 
                       bg-[var(--glass-bg)] backdrop-blur-xl border border-[var(--glass-border)]"
          >
            {/* Icon Container with Inner Glow */}
            <div className="relative z-10 p-3 md:p-6 rounded-xl md:rounded-[2rem] group-hover:scale-110 bg-[var(--glass-bg)] group-hover:bg-[var(--accent-color)]/10 transition-all duration-500 shadow-2xl">
               <motion.div 
                 animate={{
                   opacity: [0, 0.5, 0],
                   scale: [0.8, 1.2, 0.8]
                 ...
                 className="absolute inset-0 bg-[var(--accent-color)]/20 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" 
               />
            ...
            {/* Skill Name */}
            <h3 className="relative z-10 text-[10px] md:text-xs font-black text-center tracking-widest uppercase font-display transition-colors text-[var(--text-tertiary)] group-hover:text-[var(--text-primary)]">
              {skill.name}
            </h3>
            
            {/* Subtle Gradient Glow on Hover */}
            <div className="absolute inset-0 bg-gradient-to-b from-transparent to-[var(--accent-color)]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
          </motion.div>
        ))}
      </div>
    </section>
